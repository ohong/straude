...
- **Commit message linting.** Installed `@commitlint/cli` + `@commitlint/config-conventional` with a `.githooks/commit-msg` hook that enforces conventional commit format (`type: subject`). Allowed types: feat, fix, docs, chore, refactor, test, ci, perf, style, build, revert. Max header length 100 chars. No subject-case rule to allow proper nouns and camelCase identifiers. Hooks activate automatically on `bun install` via a `prepare` script.
