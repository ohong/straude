...
- **Straude CLI 1.0 agentsview collector path.** `packages/cli` is now versioned `1.0.0` and can use agentsview >= 0.26.1 for Claude Code collection while preserving Straude's native Codex collector for Codex accuracy. `STRAUDE_COLLECTOR=auto` probes agentsview with a short timeout and falls back to the legacy ccusage + native Codex path when agentsview is missing/outdated or when the one-time native Codex repair is pending. `STRAUDE_COLLECTOR=agentsview` requires a supported agentsview install; `STRAUDE_COLLECTOR=legacy` forces the old path. Added tolerant agentsview JSON parsing, missing-`totalTokens` fallback, collector metadata support, and docs in `docs/agentsview-cli-1.0-migration-plan.md`.
- **Codex-only-user fallback in the agentsview collector branch.** A user with agentsview installed but no `~/.claude/` directory previously crashed the CLI under default `auto`. `straude push` now mirrors the legacy ccusage carve-out: agentsview errors that match a conservative "no Claude data" pattern downgrade to Codex-only sync with `No Claude Code data found locally; syncing Codex usage only.`, while genuine agentsview failures still exit fatally.
- **Collector-mode telemetry on `usage_pushed`.** PostHog `usage_pushed` events now include `collector_mode`, `collector_preference`, `agentsview_available`, `codex_repair_pending`, and (when probed) `agentsview_version` so we can dashboard rollout shape and detect parity drift across the fleet. Telemetry calls are wrapped in try/catch — they cannot break a push.
- **`straude push --debug` exposes collector provenance.** Debug output now includes agentsview availability + version (or `(probe failed)`), the codex-native repair flag and timestamp, the IANA timezone we pass to agentsview, and the offline-pricing mode. Helps users and support diagnose why a particular collector path was chosen.
- **Manual web import accepts both ccusage and agentsview JSON.** `/settings/import` and the post composer now parse the modern `{ daily, totals }` shape (current ccusage v18 + agentsview) in addition to the legacy `{ "type": "daily", "data": [...] }`. Updated copy points users at either `ccusage daily --json --since YYYYMMDD --until YYYYMMDD` or `agentsview usage daily --json --since YYYY-MM-DD --until YYYY-MM-DD`. The verified-vs-unverified caveat is unchanged.
- **Agentsview version parser tolerates pre-release and build suffixes.** `parseAgentsViewVersion` now accepts `0.26.1-rc.1`, `0.27.0+build.1`, etc.; the comparator strips both before numeric comparison so a release candidate of a supported version passes the gate.
...
- **CLI telemetry now flows from end-user machines.** Embedded the public PostHog project key (`phc_*`, write-only by design — same key as the web bundle) in `packages/cli/src/lib/posthog.ts` so the CLI no longer requires users to set `POSTHOG_API_KEY` to participate. Events captured: `login_completed`, `auto_push_enabled`, `auto_push_disabled`, `usage_pushed`, `usage_push_failed`, plus uncaught exceptions — all stamped with `cli_version`. Anonymous (pre-login) users get a stable per-machine UUID at `~/.straude/machine_id` so they don't collapse into a single `"anonymous"` distinct_id; on `login_completed` we `posthog.alias()` the machine UUID to the username so anonymous events get attributed to the account. A `before_send` hook scrubs the user's home directory from any string in the payload (covers stack traces and error messages) so file paths never leave the machine. Honors `STRAUDE_TELEMETRY_DISABLED=1` and the unix-standard `DO_NOT_TRACK=1` for opt-out, and is auto-disabled under `VITEST` / `NODE_ENV=test`. Documented in `packages/cli/README.md`.
...
- **CLI handles broken pipes gracefully.** Top-level `process.stdout.on('error', …)` and `process.stderr.on('error', …)` handlers in `packages/cli/src/index.ts` exit cleanly on EPIPE instead of throwing — the previous behavior fired a `write EPIPE` exception once for every active CLI user (48 users / 48 events in the last 7 days) when output was piped to a process that closed early (`straude --help | head`).
- **CLI filters out-of-window backfill dates client-side instead of letting the whole batch fail.** `pushCommand` in `packages/cli/src/commands/push.ts` now mirrors the server's 30-day check (new exported `isWithinBackfillWindow` helper) and drops dates outside the window with a single `Note: skipping N date(s) outside the 30-day backfill window` log line. Previously, ccusage occasionally returned an entry on the boundary of the window and the server's `/api/usage/submit` route rejected the entire submit with HTTP 400 ("Date X is outside the 30-day backfill window") — 13 events across multiple users in the last 7 days.
- **"Grow Your Crew" referral CTA moved from the right sidebar to the left sidebar.** Per @bern's community-prompt request: the right rail was already crowded enough to scroll on shorter viewports while the left rail had vertical slack between Latest Activities and the bottom-anchored All-Time stats block. The block now sits between Latest Activities and the All-Time footer in `Sidebar.tsx` (and in the suspense skeleton in `app/(app)/layout.tsx`), so it fills that gap and the right rail no longer needs to scroll. `RightSidebar` no longer takes a `username` prop.
- **`docs/agentsview-migration-exploration.md` rewritten as a decision-grade brief.** Restructured around the Minto Pyramid Principle for an engineering leader: recommendation up top (greenlight a phased, additive migration; Phase 0 is the `/api/usage/submit` 5xx alert from the 2026-04-28 collector_meta incident retro), three load-bearing reasons (north-star metric leak from non-Anthropic/non-OpenAI models that LiteLLM-via-agentsview fixes for free; ~500 lines of `codex-native.ts` retired; multi-agent strategic option for Cursor/Copilot/Gemini/Warp/etc. created at copy-update cost), explicit cost-benefit table, kill-switch (`STRAUDE_COLLECTOR=legacy` env-var flip), and a "what would change my recommendation" decision aid. Updated maintainer signal (wesm/agentsview at 869⭐, v0.26.0 released 2026-04-29 — stronger long-term bet than the prior doc's "single-maintainer with unknown release cadence"). Updated install paths (pip/uvx removed in v0.26; only `curl \| bash` and desktop now — reinforces the "additive, ccusage stays as fallback" approach). Original technical scope preserved in appendices.
- **ProductHuntBadge now uses the official Product Hunt embed.** Replaced the custom medal/SVG badge with PH's official `<a>`/`<img>` embed (`featured.svg?post_id=1114059&theme=dark`) so the badge stays in sync with our launch ranking automatically. Sized at 75% of the canonical 250×54 (188×41) to fit the hero, dark-themed to match the landing page palette. Allow-listed `https://api.producthunt.com` in the `next.config.ts` CSP `img-src` directive — without that, the badge SVG was blocked by CSP and rendered as a broken-image icon.
...
- **Collector metadata can no longer spoof trusted correction behavior from web auth.** `/api/usage/submit` now validates collector metadata keys/values and only treats `straude-codex-native-v1` as a trusted lowering collector when the request authenticated through the CLI token path. `agentsview-v1` is accepted as provenance but is not trusted to lower totals until Codex parity is proven. Now covered by an explicit regression test that simulates a Supabase-web session with a `straude-codex-native-v1` payload trying to overwrite a higher-cost device row — the `mayOverwriteDevice` guard blocks the lowering and preserves the existing total.
- **Privacy and CLI landing copy now credit agentsview alongside ccusage.** `PrivacyPledge`, `/privacy`, and the `/cli` Data Sources section describe the CLI 1.0 collector model honestly (auto prefers agentsview ≥ 0.26.1 for Claude when installed, falls back to ccusage; Codex always uses Straude's native collector) without weakening the privacy pledge — the only thing leaving the machine is still aggregated daily totals.
- **Feed screenshots were cropped/zoomed inside their thumbnail tiles.** `ImageGrid` rendered every variant (single, two, three, four, four+with-overflow) with `object-cover`, so text-heavy screenshots — the default Claude Code share — got their edges chopped and looked blurry because the feed was effectively zooming into a sub-region. Switched all five rendering branches to `object-contain` via a shared `FeedImage` component, and gave each tile a subtle `bg-subtle` + `border-border/35` frame so contained images have a clean letterbox boundary instead of bleeding into the card. Lightbox click indices and aspect ratios are unchanged. Component coverage in `apps/web/__tests__/components/ImageGrid.test.tsx` asserts `object-contain` is applied and that clicking image 2 opens the lightbox at index 1.
- **Active streak silently fell back to 0 in the sidebar and recap card (#100).** Same root cause as the leaderboard fix above: migration `20260413034500_harden_users_public_columns.sql` revoked the table-wide `GRANT SELECT ON public.users` and replaced it with a column-level grant that did not include `timezone`. `calculate_user_streak` reads `users.timezone` to compute "today" in the user's local zone, but the function ran with the caller's role (no `SECURITY DEFINER`), so any call from the authenticated/anon role (sidebar via `createClient()`, public recap card) hit `42501: permission denied for table users` and the API surface dropped to `data: null` → UI rendered 0 / "No active streak". The profile page (`apps/web/app/(app)/u/[username]/page.tsx`) was unaffected because it uses the service-role client. Migration `20260430172022_fix_calculate_user_streak_security_definer.sql` promotes the function to `SECURITY DEFINER` (with `SET search_path = public`) so it can read `users.timezone` regardless of caller, and re-grants `EXECUTE` to authenticated/anon. Also routed the recap page through the service client when fetching `streak_freezes` and threaded the value through `getRecapData` so the recap streak honors the same freeze count as the profile (previously it always passed `p_freeze_days = 0`). Reported by @joledev (17-day streak on `/u/joledev`, 0 days on the recap card, "No active streak" in the sidebar; timezone `America/Tijuana`, freezes 0).
- **Direct message history can load beyond the first page.** The `/api/messages` endpoint had been changed to return 50 messages plus a `has_more` flag and `before` cursor support, but `MessagesInbox` still fetched only the first page and discarded `has_more`, making older messages inaccessible in long conversations. The inbox now surfaces a "Load earlier messages" control, fetches with `before=<oldest created_at>`, prepends de-duplicated older messages without snapping back to the bottom, and SSR preload now uses the same 50-message page contract. The API queries `limit + 1` rows so `has_more` is exact instead of true when the result length is exactly the page size.
- **Sessions running non-Claude/GPT models showed `$0.00` instead of the model.** `ccusage` only prices Anthropic models, so Claude Code sessions routed through DeepSeek, Qwen, etc. land in `daily_usage` with `cost_usd = 0` and `model_breakdown[].cost_usd = 0` despite real token counts. `formatModels` in `apps/web/components/app/feed/ActivityCard.tsx` previously bailed out (`return null`) when total breakdown cost was 0, hiding the model name entirely; the stats grid then rendered a misleading `$0.00`. Now `formatModels` falls back to a deduplicated list of pretty model names when no pricing is available, and the Cost cell renders an em-dash with a `Pricing soon` subtitle (plus tooltip "Cost tracking for non-Claude/GPT models is coming soon"). Reported by @bern after spotting danielperez's DeepSeek v4 sessions on the feed.
- **All-time stats RPC was returning input + output + cache instead of just output tokens.** Production DB still had the older definition of `get_user_usage_totals` (`SUM(daily_usage.total_tokens)`) — migration `20260406074500_fix_get_user_usage_totals_output_tokens.sql` had been committed but never applied, so the left and right sidebars showed ~7B "Output tokens" while leaderboard/profile (which read `output_tokens` directly) showed the correct ~20.4M for the same user. Applied the missing migration to alias `total_tokens` back to `SUM(output_tokens)`, and patched the `loadUsageTotals` fallback in `apps/web/app/(app)/layout.tsx` to sum `output_tokens` instead of `total_tokens` so the bug can't recur if the RPC ever fails. Reported by @bern.
- **`/leaderboard` rendered "No entries yet." for everyone.** The four `leaderboard_*` views are `security_invoker = on`, so they execute with the caller's grants. Migration `20260413034500_harden_users_public_columns.sql` replaced the table-wide `GRANT SELECT ON public.users` with a column-level grant — and `onboarding_completed` (used in the views' `WHERE` clause) wasn't on the allow-list, so anon/authenticated callers hit `42501: permission denied for table users`. The page (`apps/web/app/(app)/leaderboard/page.tsx`) ignored the error and fell back to an empty array. Migration `20260428120000_leaderboard_drop_onboarding_filter.sql` recreates all four views without the `onboarding_completed = true` predicate; `is_public = true` is the source of truth for leaderboard inclusion.
- **Sidebar "Latest Activities" and "sessions without details" nudge surfaced the wrong days after a backfill.** Both queries (`apps/web/app/(app)/layout.tsx`, `apps/web/app/api/feed/route.ts`, `apps/web/app/(app)/feed/page.tsx`) ordered posts by `posts.created_at` and limited to 3/5. When the codex-native repair backfilled 30 days, every backfilled post got a `created_at` within the same second, so the sub-second tiebreaker picked an essentially-random subset — e.g. a user whose past week was fully synced still saw the nudge listing only Apr 24, 23, 19, 22, 20 and the sidebar showing Apr 28, 24, 23. Switched all three queries to order by `daily_usage.date DESC` (with `!inner` join + `referencedTable: "daily_usage"` so PostgREST applies it to the parent rows), keeping `created_at DESC` as a tiebreaker.
- **Invite-link OG preview missing the user's avatar.** `apps/web/app/(landing)/join/[username]/opengraph-image.tsx` only allowed Supabase-hosted avatar URLs via `isFirstPartyPublicStorageUrl`, but ~53% of users (240/450) have GitHub avatars (`avatars.githubusercontent.com`) from OAuth sign-in, so their invite cards rendered the gray-letter placeholder instead. Added `isAllowedAvatarUrl` in `lib/storage.ts` that allows Supabase storage avatars plus the `https://` hosts already trusted by `next.config.ts` and the CSP (`avatars.githubusercontent.com`, `unavatar.io`, `api.dicebear.com`). The "$0 spent this week" originally reported was a stale Twitter/iMessage cached preview from a moment when the user's `daily_usage` rows weren't synced yet — the live OG render returns the correct numbers (verified via curl).
- **Right and left sidebars rendered a horizontal scrollbar.** The `<aside>` panels in `ResponsiveShellFrame` used `overflow-y-auto` while wrapping a fixed-width inner div pinned to `var(--app-{left,right}-panel-width)`. Per CSS spec, `overflow-y: auto` causes `overflow-x` to compute to `auto`, so any time the aside shrank below the inner div's width — during the collapse transition, or when a long unbroken token slipped past `truncate` — a horizontal scrollbar appeared. Added `overflow-x-hidden` to both asides. Reported by @bern via the community prompts widget.
- **CLI `usage submit` failing with `Could not find the 'collector_meta' column of 'device_usage' in the schema cache`.** Six pending migrations (dated 2026-04-13 through 2026-04-27) had been committed but never pushed to the linked Supabase project, including `20260424133000_add_usage_collector_meta.sql` which adds the `collector_meta jsonb` column referenced by `apps/web/app/api/usage/submit/route.ts`. Pushed all pending migrations via `supabase db push --linked`; PostgREST schema cache reloaded automatically.
- **PostHog $pageview events were silently dropped in production.** `PostHogPageView` from `@posthog/next` rendered as a sibling of the `useEffect` that called `posthog.init()`, so the initial pageview fired against an uninitialized client and was lost (no batched POSTs to `/ingest/i/v0/e/`). Replaced it with a local `PageviewTracker` that gates on a `ready` state set after `posthog.init()` returns, then captures `$pageview` for the initial mount and on every `usePathname` / `useSearchParams` change. Wrapped in `<Suspense>` so `useSearchParams` doesn't bail out the static shell.
