...
- **Codex collector token accounting (v1 → v2).** Two bugs collapsed into one user-visible symptom:
  ...
  Net effect on the May-4 ohong reference case: Codex/gpt-5.5 cost from real session data goes from $7,743 → $228.68. Bumped `CODEX_NATIVE_COLLECTOR` to `straude-codex-native-v2` and updated the server-side `TRUSTED_CODEX_COLLECTOR` in lockstep so v2 CLI uploads can lower totals on UPSERT. Added `codex_native_v2_repair_completed_at` so users who already ran the v1 backfill get a 30-day re-collection on their next `straude` run. New regression test (`uses last_token_usage to bill per-request — context-prune oscillation must not inflate`) plus 3 other new tests; all 276 CLI tests pass.

### Changed

- **North Star total dropped from $515,225.95 → $157,264.61 after retroactively repairing 2,683 legacy rows + 418 native-v1 rows.** Two SQL migrations re-classified historical inflated daily/device usage rows under inclusive-cache semantics and rescaled per-model `model_breakdown[*].cost_usd` by the ratio of corrected-vs-original cost estimates. Legacy migration (`20260506120100_repair_legacy_codex_inflation.sql`) targeted `collector_meta IS NULL` rows where `cache_read_tokens > input_tokens`; native-v1 migration (`20260506120200_repair_native_v1_codex_inflation.sql`) applied the same fix to `collector_meta->>'codex' = 'straude-codex-native-v1'` rows. Each repaired row is tagged with `collector_meta.repair`, `previous_cost_usd`, `previous_input_tokens`, `previous_cache_read_tokens`, `repaired_at` and gets `is_verified = TRUE` after the fix. Both migrations are idempotent, mirror corrections onto `device_usage` (with rounding-drift reconciliation onto the most-recently-updated device row so device sums match daily totals to the cent), refresh all four leaderboard materialized views, and write per-row entries to a new `corrections_log` audit table. Trust-over-vanity: the corrected total reflects what users actually spent.

- **New `corrections_log` audit table (`20260506120000_create_corrections_log.sql`).** Records every retroactive cost correction with `(user_id, date, table_name, reason, previous_values jsonb, new_values jsonb, created_at)`. RLS read-only for the row's owner; service-role writes via migrations. First users: the two repair migrations above (5,578 rows logged: 2,683 daily_usage + 2,895 device_usage from the legacy run, plus the v1-native population).

- **Post card surfaces a "Repaired (was $X.XX)" hint when `daily_usage.collector_meta.repair` is set.** Lives below the Cost cell in `apps/web/components/app/feed/ActivityCard.tsx`. Tooltip explains that cache-vs-uncached token accounting was repaired retroactively. Kudos and comments stay attached — only the cost number changes.
