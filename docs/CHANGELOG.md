...
- **Straude CLI agentsview migration pinned to v0.28.0.** The agentsview branch now requires agentsview >= 0.28.0, the latest stable upstream release as of 2026-05-08. `straude push` routes through one unfiltered agentsview daily-usage call for every supported coding agent, submits `collector.unified = "agentsview-v1"`, and leaves the old ccusage/native Codex collector modules dormant for a fast revert.
- **Collector-mode telemetry on `usage_pushed`.** PostHog `usage_pushed` events now include `collector_mode = "agentsview-unified"` and `agentsview_version` so we can dashboard rollout shape. Telemetry calls are wrapped in try/catch: they cannot break a push.
- **`straude push --debug` exposes collector provenance.** Debug output now includes agentsview version, the IANA timezone we pass to agentsview, and the offline-pricing mode. Helps users and support diagnose the collector path without exposing prompts, paths, or raw logs.
...
- **Replaced GLOBAL_FEED.LOG with Privacy Pledge on landing page.** New section 03 ("Privacy by architecture") lists what Straude cannot access (prompts, code, transcripts), explains the local collector pipeline, links to the open-source CLI and full privacy policy. Removed `GlobalFeed` component from the landing page.
...
- **Collector metadata can no longer spoof trusted correction behavior from web auth.** `/api/usage/submit` now validates collector metadata keys/values and only treats trusted collector provenance as lowering authority when the request authenticated through the CLI token path. Current CLI clients submit `collector.unified = "agentsview-v1"`; older native-Codex repair clients can still submit `collector.codex = "straude-codex-native-v1"`. A regression test simulates a Supabase-web session with trusted-looking collector metadata trying to overwrite a higher-cost device row — the `mayOverwriteDevice` guard blocks the lowering and preserves the existing total.
- **Privacy and CLI landing copy now credit agentsview as the default collector.** `PrivacyPledge`, `/privacy`, and the `/cli` Data Sources section describe the agentsview-backed CLI model honestly without weakening the privacy pledge: the only thing leaving the machine is still aggregated daily totals.
