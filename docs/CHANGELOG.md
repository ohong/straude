...
- **Admin dashboard (`/admin`).** Single-page dashboard for tracking the North Star Metric (cumulative spend), user activation, and engagement. Includes: stat cards (Total Spend, Total Users, DAU/WAU/MAU), cumulative spend area chart with 7D/14D/30D/All zoom, activation funnel (Signed Up → Onboarded → First Usage → First Post → 3d Retained), user growth chart, and top-20 users table by spend. Server components fetch via Supabase service client (bypasses RLS) through four `SECURITY DEFINER` RPCs. Access restricted to user IDs in `ADMIN_USER_IDS` env var. Charts powered by recharts (lazy-loaded on `/admin` route only). Light/dark theme toggle (persisted to localStorage) with Benji Taylor–inspired card aesthetic: centered stat numbers with dot labels, 12px-radius cards, whisper-quiet borders, theme-scoped CSS variables.
