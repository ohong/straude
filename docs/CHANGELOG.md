...
- **Straude CLI agentsview migration pinned to v0.28.0.** The agentsview branch now requires agentsview >= 0.28.0, the latest stable upstream release as of 2026-05-08. The migration docs now target agentsview as the single local collector for every supported coding agent, replacing both ccusage and Straude's native Codex collector instead of preserving the earlier hybrid plan.
...
- **Agentsview version parser tolerates pre-release and build suffixes.** `parseAgentsViewVersion` now accepts `0.28.0-rc.1`, `0.28.0+build.1`, etc.; the comparator strips both before numeric comparison so a release candidate of a supported version passes the gate.
...
- **`docs/agentsview-migration-exploration.md` rewritten for the agentsview-only migration.** The brief now explicitly rejects the older ccusage/native-Codex hybrid as the wrong ownership boundary, pins the plan to agentsview v0.28.0, documents the all-supported-agents collector invocation, and lists upstream agentsview issues we should monitor instead of reimplementing locally.
...
- **Privacy and CLI landing copy now credit agentsview alongside ccusage.** `PrivacyPledge`, `/privacy`, and the `/cli` Data Sources section describe the transitional CLI collector model honestly (auto prefers agentsview >= 0.28.0 for Claude when installed, falls back to ccusage; Codex still uses Straude's native collector until the agentsview-only migration lands) without weakening the privacy pledge: the only thing leaving the machine is still aggregated daily totals.
