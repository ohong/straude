...
- **North Star repair sequence: $515,225.95 → $157,264.61 on the first Codex-inflation pass, then back to $333,815.74 after restoring legitimate Claude spend.** The first two SQL migrations re-classified historical inflated daily/device usage rows under inclusive-cache semantics and rescaled per-model `model_breakdown[*].cost_usd` by the ratio of corrected-vs-original cost estimates. The follow-up Claude restore put untouched ccusage-derived Claude cost back, and the v3 Codex-only repair scales only Codex breakdown entries while preserving Claude entries. These migrations are heuristic for mixed rows because historical per-model token splits are not stored; re-pushing with the fixed Codex collector is the authoritative Codex correction path.

- **New `corrections_log` audit table (`20260506120000_create_corrections_log.sql`).** Records every retroactive cost correction with `(user_id, date, table_name, reason, previous_values jsonb, new_values jsonb, created_at)`. RLS read-only for the row's owner; service-role writes via migrations. First users: the two repair migrations above (5,578 rows logged: 2,683 daily_usage + 2,895 device_usage from the legacy run, plus rows tagged with the previous native Codex collector marker).

- **Post card surfaces a "Repaired (was $X.XX)" hint when repair metadata is present.** Lives below the Cost cell in `apps/web/components/app/feed/ActivityCard.tsx`. Tooltip copy is reason-specific and audit-oriented, covering legacy repairs, v3 Codex-only repairs, and Claude-restore metadata without implying Claude usage was recalculated by the Codex path.
