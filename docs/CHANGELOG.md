...
- **Codex usage repair preserves mixed-model data.** Trusted Codex repair submissions can lower a row only when the entry contains Codex usage and its `model_breakdown` proves non-Codex cost is preserved. Claude-only rows, mixed rows with lower Claude cost, and previously repaired rows from stale v1 clients are left intact.
