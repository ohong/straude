...
- `__tests__/commands/sync.test.ts` — Tests for sync command (6 cases: first login, no prior push, already synced, diff push, backfill cap, login failure).
- `__tests__/flows/cli-sync-flow.test.ts` — Integration flow tests (22 cases) mocking only at boundaries (fetch, fs, ccusage binary). Covers: full sync flows, API error handling (404/401/500/network), endpoint path verification, config persistence, ccusage failures.
