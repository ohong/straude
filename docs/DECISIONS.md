...
**Discovery:** Fetch `/favicon.svg` and the origin document concurrently. A usable conventional SVG cancels the remaining search. Extract HTML links with parse5-sax-parser without building a document DOM, honoring the redirected document URL, the first base URL, case-insensitive rel tokens, icon metadata and declared manifests. Ignore inert template content and stop after 2,048 start/end tags or a 100 ms parsing budget checked between 1,024-character chunks. Direct parsing failures still allow the Google fallback. Try known HTML SVG candidates before fetching manifests so a stalled manifest cannot consume their deadline. Accept the first usable SVG in this deterministic order; any mislabeled raster result remains cached while the search continues. Resolve manifest icon URLs against the final manifest URL. Prefer SVG, then default-purpose and default/light-media variants, using decoded raster dimensions to break quality ties. Preserve a usable raster while checking for SVG. If declared candidates fail, probe `/favicon.png`, `/manifest.webmanifest`, `/manifest.json` and `/app.webmanifest`; use Google's `s2/favicons?domain=<domain>&sz=128` endpoint only if direct discovery produces no usable icon. Skip declared ICO URLs and MIME types and reject ICO bodies; there is no local ICO decoder. Open Graph images describe content previews and are not favicon candidates.

**Bounds and trust:** Allow HTTP(S), default ports and no credentials. Every destination, including cross-origin redirects, must resolve entirely to public addresses. Node's request lookup connects to the checked IP while preserving the URL hostname for Host and TLS; there is no second DNS lookup. Block special/reserved ranges and cloud metadata destinations. Permit at most two concurrent downloads, 24 direct network requests including redirects, three redirects per resource, two declared manifests, and 12 ranked icons per discovery stage. Direct discovery has a five-second network budget, including DNS and streaming reads. The Google fallback has a separate two-second budget and up to three redirects under the same public-IP and streaming protections; the resolver's ten-second deadline also covers cache and Storage requests. Accept at most 512 KiB per document and 4 MiB per source image. Direct downloads share a 12 MiB cumulative body budget, charged during streaming even when a resource is rejected; Google has a separate 4 MiB allowance. The per-search cache retains normalized images, extracted document candidates or misses, never the original response buffers. Store each original byte count so reuse still respects the consuming resource's limit. Reject compressed HTTP bodies rather than expanding an unbounded payload.

**Image handling:** SAX parsing rejects SVG document types, custom entities and processing instructions, with at most 2,048 elements, 32 levels and 64 attributes per element. DOMPurify removes active/unsupported markup; CSS Tree validates permitted styling and fragment-only resource references. SVG remains vector output and never goes through sharp. Raster signatures are checked before decoding. Sharp limits inputs to 16,777,216 pixels, applies orientation, and resizes inside 128×128 using Lanczos3 without enlargement. PNG uses compression level 6 and no palette quantization: resizing removes detail, while PNG encoding preserves the normalized pixels. Output must fit the bucket's 512 KiB limit. Google responses must decode as raster images and pass the same PNG normalization and output limits.

**Cache:** The private `team_favicon_cache` table maps normalized domains to actual object paths or 15-minute retry times after both direct discovery and Google fail. Google results are stored in the same bucket and reused like direct results; browsers never request Google. Successful results do not expire. Existing `<domain>.png` objects are reused and lazily recorded; no mass refresh is needed. Negative writes cannot replace a concurrent successful entry. An unrelated settings save reads the current team fields only when a nonempty team URL is supplied and bypasses resolution when the normalized team and resolved icon are unchanged. The team input stays disabled until React has attached its handlers, preventing text entered during hydration from being reset before submission. A valid team URL still saves if discovery or Storage fails.

**Alternatives and costs:** Using Google for every lookup makes availability and image quality depend on that service. Prefer direct icons and retain Google only as a compatibility fallback. Local ICO decoding would add an unmaintained dependency (`decode-ico` last released in 2022); omit it and let the fallback cover sites whose usable icon is only ICO. Direct fetching still adds DNS, parsing, sanitizer and raster decoder maintenance. Exhaustive crawling adds latency without a reliable stopping point; this bounded search can miss icons beyond its budget. A process-local cache would disappear across serverless workers, while probing every possible Storage extension cannot remember misses. Persistent metadata handles both cases. Periodic refresh, custom uploads and domain ownership verification remain separate work.

**Verification:** Run `bun run --cwd apps/web test:favicons`, then `bun run --cwd apps/web test:integration __tests__/integration/team-favicon.test.ts` against migrated local Supabase. The integration test uses a controlled external transport and real authentication, database and Storage; the transport suite separately checks public-address validation, pinned lookup, redirects, streaming caps and cancellation with a local HTTP fixture. Run `bun run --cwd apps/web test:e2e e2e/team-favicon-save.spec.ts --reporter=list` for authenticated settings saves, actual SVG/PNG browser decoding, rectangular proportions and persistence. Tests create and remove only their own users, metadata and objects.

**References:** https://github.com/cure53/DOMPurify, https://github.com/csstree/csstree, https://nodejs.org/api/http.html, https://github.com/whitequark/ipaddr.js, https://sharp.pixelplumbing.com/api-resize/, https://sharp.pixelplumbing.com/api-output/#png, https://www.w3.org/TR/appmanifest/#icons-member.
...
**Decision:** All supported coding-agent ingestion runs through a single installed `ccusage` v20 dependency invoked as a native binary. The current floor is `20.0.20`; Straude's native collectors, token normalizer, source whitelist, and pricing aliases are deleted, so Straude only parses ccusage's unified daily JSON into storage rows.
...
- `old_estimate = Σ_m (input_share_m × input_rate_m + cache_share_m × cache_read_rate_m + output_share_m × output_rate_m)` over the inflated buckets — this reproduces the recorded cost.
...
**Decision:** In `packages/cli/src/lib/codex-native.ts:normalizeBucket`, replace the call to the shared `normalizeTokenBuckets({source:"codex", cacheSemantics:"subset_of_input"})` (which builds two candidates — "separate cache" vs "inclusive cache" — and picks based on consistency error against `total_tokens`) with a direct, hard-coded `cache_read = min(cached, input); input = input - cache_read` clamp. Emit a `NormalizationAnomaly` with `confidence: low` when source data violates the invariant rather than silently letting cache exceed input.

**Why:** Codex's documented schema (OpenAI Responses API) says `cached_input_tokens` is a subset of `input_tokens`, full stop. There is exactly one valid interpretation; the dual-candidate logic exists for sources where semantics are genuinely ambiguous (`ccusage` for Claude). Under bucket-level summation across multi-event sessions, the dual-candidate consistency check could pick the "separate" candidate when arithmetic drift made it look more consistent — and once "separate" wins, there's no clamp, so cache_read can exceed input by orders of magnitude. At gpt-5.5's $0.50/M cache-read rate, billions of phantom cache tokens added thousands of dollars per row.

**Alternatives considered:**

1. **Add a `subset_of_input_strict` mode to `normalizeTokenBuckets` that always picks the inclusive candidate.** Cleaner separation, but it concentrates Codex-specific semantics in a generic util that doesn't otherwise know about Codex. Rejected — the codex path is short enough to inline the clamp directly, and the inline version is easier to read and harder to misuse.
...
The smoking-gun numbers, all from the same May-4 session file (3,691 token_count events):

| Interpretation | input | cached | output |
|---|---|---|---|
| End-of-session cumulative | 148.8M | 144.5M | 344.9k |
| Sum of `last_token_usage` (no dedup) | 525.8M | 511.0M | 1.21M |
| Sum of cumulative deltas (legacy logic) | **10,368.9M** | 10,006.8M | 29.1M |
| Sum of `last_token_usage` w/ dedup (new logic) | ≈296M | ≈296M | ≈782k |

Pricing the new-logic numbers at gpt-5.5 rates: $228.68 — matches what OpenAI actually billed.
