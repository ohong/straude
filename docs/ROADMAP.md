...
Direct favicon discovery shipped on 2026-09-05. It prefers sanitized SVG, uses declared HTML and manifest icons, and normalizes raster images to proportionally resized PNG within a finite request budget. If direct discovery fails, Google supplies the final fallback. HTML extraction avoids a full DOM, known SVG candidates run before manifests, and downloads have a cumulative byte budget. Assets stay in Supabase Storage; ICO decoding and Open Graph preview images are excluded. The design and limits are recorded in `docs/DECISIONS.md`.

- **Periodic favicon refresh.** Successful discoveries and legacy PNGs remain cached until explicitly invalidated. Re-saving an unchanged, resolved team does not refresh its icon. A weekly cron that re-fetches favicons for distinct team domains would keep rebrands current without per-user action.
...
- **Extend deterministic badge surface coverage.** `e2e/team-favicon-save.spec.ts` now signs in a temporary local user, saves SVG and rectangular PNG cache hits through settings, checks browser decoding and persistence, and verifies the public profile. Extend this fixture to feed cards and populated leaderboard rows; the older public-surface spec can still skip when its target has no team.
