...
export default function SettingsClient({
  ...
  const [heardAbout, setHeardAbout] = useState(initialProfile.heard_about ?? "");
  const [link, setLink] = useState(initialProfile.link ?? "");
  const [country, setCountry] = useState(initialProfile.country ?? "");
  const [githubUsername, setGithubUsername] = useState(initialProfile.github_username ?? "");
  const [teamUrl, setTeamUrl] = useState(initialProfile.team_url ?? "");
  ...
  const [emailMentionNotifications, setEmailMentionNotifications] = useState(initialProfile.email_mention_notifications ?? true);
  const [emailDmNotifications, setEmailDmNotifications] = useState(initialProfile.email_dm_notifications ?? true);
  const [timezone, setTimezone] = useState(initialProfile.timezone ?? "");
  const crewCount = initialProfile.crew_count;
