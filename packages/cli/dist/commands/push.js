...
import { runCcusageRaw } from "../lib/ccusage.js";
...
export async function pushCommand(options) {
    ...
    console.log(sinceDate.getTime() === untilDate.getTime()
    ...
    for (const entry of entries) {
        ...
        console.log(`    Tokens: ${formatTokens(entry.totalTokens)} (input: ${formatTokens(entry.inputTokens)}, output: ${formatTokens(entry.outputTokens)})`);
    ...
    let response;
    try {
        response = await apiRequest(config, "/api/usage/submit", {
            method: "POST",
