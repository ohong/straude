 ...
 */
export function runCcusage(sinceDate, untilDate) {
    ...
    try {
        ...
        if (error.status === 127 || (error.stderr && error.stderr.includes("not found"))) {
            throw new Error("ccusage is not installed. Install it with: npm i -g ccusage");
        }
        throw new Error(`ccusage failed: ${error.stderr ?? "unknown error"}`);
    }
    return parseCcusageOutput(stdout);
}
export function parseCcusageOutput(raw) {
    let parsed;
    try {
 ...
 */
export function runCcusageRaw(sinceDate, untilDate) {
    const args = ["daily", "--json", "--since", sinceDate, "--until", untilDate];
    try {
